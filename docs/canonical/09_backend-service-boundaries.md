...
**MVP implementation (DEC-061 + DEC-090)**: Deterministic templating for ALL match types is the default and the airtight fallback. Slice K (DEC-090) closes Bucket 7 by adding an **optional, injected, conceptual-match-only** LLM paraphrase path. When `explain()` is called with `llm_client: LLMClient` AND a candidate has `match_type == "conceptual"`, that candidate's `explanation` field is paraphrased by the LLM from grounded structured fields (verse reference, sequence label, match type, per-step lemma + node value + resolved lemmas). All other surfaces — `summary`, `contextualization` baselines, alt-ordering phrases, `validation_notes`, and the per-candidate prose for `match_type in {"exact", "variant"}` — remain deterministic.
...
- (b) Every prose claim derives from fields on `result`, `plan`, or `validation` — never invented (DEC-024 corpus-is-ground-truth). Caller contract: `result.contextualization.observed_count` MUST equal `len(result.candidates)`; the explainer reads both fields and a divergence will produce internally-contradictory prose. The LLM-augmentation path strengthens this invariant rather than weakening it: the LLM's only inputs are grounded structured fields (see `src/nlp/prompts/explainer_prompt.py::build_explainer_user_message`), and the live-LLM exit gate (`tests/integration/test_explainer_llm_prose_live.py::test_llm_prose_only_contains_grounded_numbers_and_refs`) asserts every digit substring and every verse-shape token in the LLM prose traces to a grounded input field (DEC-081 conformance).
- (c) The explainer is deterministic by default; LLM-backed prose for conceptual matches is opt-in via an injected `LLMClient` and never replaces deterministic grounded fields. The deterministic helper (`_per_candidate_prose`) is the airtight fallback — any LLM failure (`LLMUnavailable`, any other `Exception`, the LLM's explicit `FALLBACK` sentinel, or empty output) returns the deterministic prose with a WARNING log. The route layer opts in via `SPL_EXPLAINER_LLM` env var (truthy: `1` or `true`, case-insensitive); the DSL surface (`run_dsl_query`) stays explicitly LLM-free. Verifiable via `grep -L "anthropic\|openai\|httpx" src/nlp/explainer.py` (the explainer imports `LLMClient` as a type-annotation reference but never constructs an anthropic/openai/httpx client directly). [DEC-061, DEC-081, DEC-090]
...
- (e) **Cap policy** (Bucket 4 closure): the explainer caps resolved-lemma display at 5 items with `"+N more"` suffix and sequence labels at 64 chars with ellipsis. Slice K adds a 300-char post-truncation cap on LLM-paraphrased prose as defense-in-depth (the system prompt requests ≤200 chars; the truncation guarantees bounded output even if the LLM ignores the prompt). The structured `_print_contextualization` block in `scripts/query.py` remains unbounded — that block is the data-fidelity view; the prose layer is where presentation discipline applies.
