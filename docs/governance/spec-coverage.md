...
Last updated: 2026-05-03 (Slice B Phase 2 — `parse_corpus_directory` multi-file unit test; full table still pending `/coverage` run)
...
| REQ:08.ingestion-pipeline | Steps for ingesting MorphGNT data into Postgres | `scripts/db/apply_schemas.sh` (step 4 mechanism); `src/ingestion/corpus_parser.py` (steps 2–3); `src/ingestion/loader.py` (step 4 bulk insert; opt-in `progress_callback` for observability) | `tests/unit/test_corpus_parser.py::TestParseCorpusFile` (steps 2–3 single-file); `tests/unit/test_corpus_parser.py::TestParseCorpusDirectory` (step 3 cross-file: BB-key sort, `global_position` continuity, `position` reset at boundary, count additivity); `tests/unit/test_loader.py::TestProgressCallback` (loader callback semantics: per-batch, file-boundary, post-commit done; `tokens_loaded` = committed count); `tests/integration/test_corpus_ingest.py::test_load_tokens_returns_219`, `test_table_row_count_is_219` (step 4 end-to-end on 3 John); `tests/integration/test_apply_schemas.py` (step 4 apply-script idempotency on fresh DB); steps 5–6 pending Slice C | DEC-021, DEC-022, DEC-023, DEC-027, DEC-028, DEC-029, DEC-034, DEC-035, DEC-036, DEC-037 |
...
| REQ:09.ingestion | Corpus ingestion component (non-request-path): parse corpus files, bulk-load tokens, manage schema apply; query-side packages must not import it | `src/ingestion/corpus_parser.py`; `src/ingestion/db.py`; `src/ingestion/loader.py` (`progress_callback` exposes loader observability without an internal logger); `scripts/db/apply_schemas.sh` | `tests/unit/test_corpus_parser.py` (incl. `TestParseCorpusDirectory` cross-file iteration via `_BOOK_NUMBER_BY_FILENAME` monkeypatch); `tests/unit/test_loader.py`; `tests/integration/test_corpus_ingest.py`; `tests/integration/test_apply_schemas.py` | DEC-021, DEC-025, DEC-028, DEC-034, DEC-037 |
