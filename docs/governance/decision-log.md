...
## DEC-102 — Concept creation is a three-tier model; Tier-1 translation mappings auto-generate (charter scoped)
- Status: Accepted (design-stable; implementation pending — Slice N reframed). Source: `thoughts/design-concept-layers-2026-05-26.md`, `thoughts/research-concept-registry-2026-05-26.md`, conversation 2026-05-26.
- Question: What happens when a user queries a term with no seeded concept, and does creating a concept require the full human-validation gate? The earlier Cut A design (`design-concept-curator-refresh-2026-05-26.md`) treated ALL concept creation as one gated propose→unverified→human-validate flow. The user reframed: that conflates two very different things.
- Decision: Concept creation is **three tiers** with different validation burdens. **Tier 1 — translation history**: a single English word ↔ the Greek lemmas usually translated as it (e.g. humility → ταπεινοφροσύνη / ταπεινός / πραΰτης). Near-lexical, abundantly evidenced, lexicon-documented. **AUTO-GENERATED** on an unmapped term from authoritative open lexicon data (DEC-103), keeping only lemmas present in the SBLGNT corpus, with the lexicon citation + corpus verses as base evidence; marked `origin='lexicon_imported'`, `verification_state='unverified'`, corrigible, NEVER auto-promoted to `human_confirmed`. **Tier 2 — conceptual equivalence/grouping**: a claim that expressions (possibly with entirely different underlying Greek, incl. multi-word phrases like "enduring to the end") "hang together" conceptually. NOT a translation fact — a hypothesis the corpus must test. Graded/weighted membership (the `confidence` column carries overlap degree; not binary). Members may be lemmas OR sub-patterns. Requires corpus evidence + a HUMAN synthesis judgment (DEC-081 bites here; the 2026-05-10 curator design is the north star). **Tier 3 — pattern-matching payoff**: a concept search expands across its Tier-2 grouping, weighted — maps onto the existing `conceptual`/`expanded` match types.
- Charter change: the CLAUDE.md Non-Negotiable "Do not auto-approve concept registry entries — all mappings are human-reviewed" is **scoped to Tier-2 conceptual claims**. Tier-1 lexicon-sourced translation mappings may auto-generate under the constraints above. DEC-024 (corpus is ground truth) preserved: Tier-1 is a sourced prior, Tier-2 claims are tested hypotheses, nothing claims `human_confirmed` it didn't earn. DEC-081 (agent proposes, never validates) is preserved and now precisely located at Tier 2.
- Rationale: (a) Tier-1 mappings are "almost citing a dictionary" — low-stakes, correctable, with abundant open evidence; gating them is friction without epistemic benefit. (b) The real intellectual work + confirmation-bias risk lives at Tier 2 (conceptual leaps), which keeps the full human-validation gate. (c) Auto-generating Tier 1 kills the `concept_not_mapped` dead-end a real user hit. (d) Phrase-valued, weighted Tier-2 membership matches how the domain actually works (the "enduring to the end" example) and feeds the existing match-type taxonomy.
...
## DEC-103 — Source Tier-1 translation mappings by self-hosting open lexicon data (not a hosted API)
- Status: Accepted (design-stable; implementation pending). Source: `thoughts/research-greek-lexicon-sourcing-2026-05-26.md` (two Sonnet web-research passes).
- Question: Where do the Greek equivalents for Tier-1 auto-generation come from — a hosted/callable API, or self-hosted open data?
- Decision: **Self-host** a tiny, permissive 3-dataset stack, ingested once (mirroring the corpus/seed ingests): **`jtauber/greek-lemma-mappings`** (CC BY-SA — the MorphGNT-lemma ↔ Strong's bridge, aligns to our corpus by construction), **STEPBible TBESG** (CC BY 4.0 — Strong's ↔ English gloss, the reverse-lookup source), **Dodson** (Public Domain — gloss fallback). Pipeline: English word → TBESG gloss match → Strong's → jtauber mapping → MorphGNT lemma → SBLGNT verses (citations). <10 MB, no runtime API dependency. An LLM may assemble/explain cited evidence but is never the source of truth (Bolls.life is an optional enrichment only).
- Rationale: research found NO hosted API implements English→Greek/Strong's reverse lookup as a clean first-class endpoint; the usable ones (API.Bible) forbid building a local lookup table (≤500-verse cache, 14-day clear) and are translation-text-focused; the closest (Bolls.life) returns prose and has no SLA. The open datasets are tiny, permissively licensed, and bridge to the existing MorphGNT corpus with zero runtime dependency — the more reliable, cheaper, more auditable route.
